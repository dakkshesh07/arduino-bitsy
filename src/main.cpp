...
static const float length_c = 27.5;
static const float length_side = 71;
...
static const float speed_multiple = 1; //movement speed multiple
...
static const float temp_c = sqrt(pow(2 * x_default + length_side, 2) + pow(2 * y_start + y_step + length_side, 2));
static const float temp_alpha = acos((pow(temp_a, 2) + pow(temp_b, 2) - pow(temp_c, 2)) / 2 / temp_a / temp_b);
//site for turn
